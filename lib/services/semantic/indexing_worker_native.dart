    ...
    if (message is _InitMsg) {
      try {
        ...
        try {
          ...
          print('[IndexingWorker] fromFile failed (possible path encoding issue on Windows), trying buffer fallback: $e');
          // 2. Fallback: read bytes first, then load from buffer.
          //    This bypasses the Windows ANSI path encoding bug in ONNX Runtime's
          //    native layer (the garbled-path error seen in logs).
          final modelBytes = await File(message.modelPath).readAsBytes();
